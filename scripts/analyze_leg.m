...
% trim images to get rid of area outside of calf
row_start = 20;
row_end = 150;
col_start = 100;
col_end = 240;

fn_epg{1} = 'MRF_leg_TBW4_C1_proc_epg.mat';
fn_epg{2} = 'MRF_leg_TBW4_C2_proc_epg.mat';
fn_epg{3} = 'MRF_leg_TBW4_C4_proc_epg.mat';

fn_pepg{1} = 'MRF_leg_TBW4_C1_proc_pepg.mat';
fn_pepg{2} = 'MRF_leg_TBW4_C2_proc_pepg.mat';
fn_pepg{3} = 'MRF_leg_TBW4_C4_proc_pepg.mat';

fn_ssepg{1} = 'MRF_leg_TBW4_C1_proc_ssepg.mat';
fn_ssepg{2} = 'MRF_leg_TBW4_C2_proc_ssepg.mat';
fn_ssepg{3} = 'MRF_leg_TBW4_C4_proc_ssepg.mat';

n_fn = numel( fn_epg );

fn_img{1} = 'MRF_leg_TBW4_C1_img_recon.mat';
fn_img{2} = 'MRF_leg_TBW4_C2_img_recon.mat';
fn_img{3} = 'MRF_leg_TBW4_C4_img_recon.mat';
...
c.Position(4) = 0.5;
c.LineWidth = 2;
caxis([0 2000]); colormap(hot);
export_fig('../figures/leg_T1.png',sprintf('-r%d',my_dpi));
...
c.Position(4) = 0.5;
c.LineWidth = 2;
caxis([0 50]); colormap(hot);
export_fig('../figures/leg_T2.png',sprintf('-r%d',my_dpi));
...
axis image; axis off;
c = colorbar('South','Color','w','FontSize',font_sz);
c.Label.String = 'B_0 (Hz)';
caxis([-45 45]); colormap(gray);
c.Position(3) = 0.5;
c.Position(1) = 0.375;
c.Label.Position(1) = -65;
c.Label.Position(2) = 0;
c.LineWidth = 2;
export_fig('../figures/leg_B0.png',sprintf('-r%d',my_dpi));
...
export_fig('../figures/leg_B0_zoned.png',sprintf('-r%d',my_dpi));
...
c.LineWidth = 2;
caxis([0 50]); 
export_fig('../figures/leg_COV.png',sprintf('-r%d',my_dpi));
