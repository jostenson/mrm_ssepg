function [Fxy,Ft] = EPG_MRF_SSFP_B0_prof_par( T1_list,T2_list,B0_list,TE_v,TR_v,FA_v,delk,nreps,szomega,phi_v,TI,rf_shape,n_T1T2,kerns,ksp_flag)
...
for ii=1:nreps
    ...
    % phase offset for B0
    omega = phase_B0( omega, TE, B0_list );
...
%     drawnow
    ...
    % phase offset for B0
    tau = TR-TE;
    omega = phase_B0( omega, tau, B0_list );
...
function w = phase_B0( w,tau,B0_v )

p_v = exp( 1i * 2 * pi * tau * B0_v(:).' );
w(1,:,:) = squeeze( w(1,:,:) ).* p_v;
w(2,:,:) = squeeze( w(2,:,:) ).* conj( p_v );
